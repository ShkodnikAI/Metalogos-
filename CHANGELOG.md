...
- **Wave-3 acceptance №1 — the kitchen camera end-to-end with grant/deny/ledger integration (naryad №397, dispatch issue #491)**: the composed runnable scenario `examples/w3_kitchen_camera_e2e.mlog` (+ golden `.expected`) walks the full story in ONE program: a private kitchen-camera frame enters the flow → the GREEN consent path materializes it through `media_save` (sealed at rest, C2PA-style sidecar manifest) → an N(1) grant meters ONE destructive `DELETE` (the granted call journals itself as `irreversible.db_execute`) → the second granted call exhausts the quota and refuses as a runtime DenyEvent `IRREVERSIBLE_NO_GRANT`, handled by `on_deny(db)` and degraded to Unit → the ledger is EXPORTED and verifies EXTERNALLY (`mlog ledger verify`, pure hash + Ed25519, no runtime). Contract: `tests/naryad_397_kitchen_camera_e2e.rs` (5 tests — TW↔VM parity for the composed story, the canonical static deny sharing the №392 deny-reason dictionary, the unconsented refusal, the revoke twin, the signed-trail assertions as monotone kind deltas). The e2e caught and fixes two integration defects the per-layer tests could not see: (1) **the layers disagreed on consent** — the static audit accepted a consent scope as the `media_save` credential (№387) while the runtime backstop knew only the likeness token, so a consent-clean compile was sealed at runtime (`MEDIA_SEALED_EGRESS`); fixed by store-aware consent interception: `consent_grant_dispatch`/`consent_revoke_dispatch` (TW statement+expression paths and the VM's `call_media_builtin`) record the scope ON THE STORE ENTRY (`MediaStore::extend_entry_consent`/`clear_entry_consent`, the ConsentScope meet — capabilities accumulate), the backstop honors a consented entry, `consent_revoke` withdraws the credential (the runtime twin of the flat cascade) and quarantine never materializes through any sink; (2) **the runtime ledger started at seq 1 with a genesis prev_hash** — the external verifier refuses any chain not starting at seq 0, so EVERY fresh runtime export was unverifiable and the dispatch's "external verification passes" was structurally impossible; fixed in `append_record` (genesis record takes `seq 0`; the empty-ledger test is the GENESIS prev-hash, not the head_seq sentinel) and `head_hash` (a one-record chain reports its real head). REFERENCE.md truth-up for `media_save`/`consent_grant`/`consent_revoke` (the two-credential gate); README resync (195 test files, 246 example programs).

...
- **Kitchen-camera e2e — the wave-3 capability layer integrated end to end (naryad #397, wave acceptance №1, gh#491)**: the §5.3 scenario (`examples/w2_kitchen_camera.mlog` + golden) carries BOTH sides of the acceptance contract in ONE flow — the GREEN path (private capture against the kitchen-camera origin → the one-time likeness ritual issues the consent credential → `media_save` passes the №325 clearance with the token; the ritual records the consent-ledger trace, the sidecar manifest carries the №332/№337 origin+C2PA facts) and the DENY path (the housekeeping purge runs under an N(1) grant — the first granted DELETE executes and journals itself, the second exhausts the quota and refuses: GRANT_EXHAUSTED → runtime DenyEvent with reason `IRREVERSIBLE_NO_GRANT`, the signed deny record lands BEFORE handler selection, `on_deny(db)` explains it via `deny_reason()` and degrades to Unit) — then rotates, snapshots and EXPORTS the chain for `mlog ledger verify`. Contract: `tests/naryad_397_kitchen_camera_e2e.rs` (4 tests — TW→VM parity with per-action ledger deltas, external `verify_file` on the exported chain, the consent-trace assertion, the unconsented-media deny at compile time + the №331 runtime backstop twin, the canonical red contracts stay red). The e2e caught and fixed THREE defects: (1) **ledger genesis off-by-one** (`src/ledger.rs`): `append_record` numbered the first record seq 1 while `verify_records` (and ADR-0167 §Genesis) demand seq 0 with the all-zeros prev_hash — EVERY runtime export failed external verification (`INVALID: record 1: chain must start at seq 0`); the first record now lands at the genesis position; (2) **static gate vocabulary** (`src/semantic.rs`): the irreversible-content matcher covered only `drop table|drop database|drop index|truncate` while its own doc comment (audit.rs), REFERENCE.md and the runtime twin (`grants.rs::extract_destructive_ops`) claimed DROP/DELETE/TRUNCATE/ALTER — bare `db_execute("DELETE FROM …")` compiled clean and executed ungranted; `delete from`/`alter table` now gate at compile time; (3) **TW↔VM ledger parity** (`src/vm.rs`): the VM's granted destructive-SQL success path wrote `grant.used` but skipped the `irreversible.db_execute` journal record the TW path writes (`src/interpreter/db.rs`) — the signed trail on the VM backend missed the very action the ledger exists for; the record now mirrors the TW detail format (grant_id|scope|sql, SHA-256 only). The gate fix realigned one stale contract: `n152_db_execute_literal_passes_check` (check_integration.rs) asserted a parameterized `DELETE` literal passes `mlog check` — the №152 SQL_DYNAMIC intent (injection guard) is preserved on a non-destructive literal, while the destructive literal now carries the layered IRREVERSIBLE_NO_GRANT contract (`n397_destructive_literal_needs_a_grant`). README anchors resynced (194→195 test files, 245→246 examples).

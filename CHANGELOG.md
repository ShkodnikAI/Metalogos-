...
closing docs release: REALITY recounted (§6.11 — the new wave counters:
the TW/VM duplication 60 → 35, the typed-signature share 49/500 = 9.80%,
the debt 85/49/37; the gate inventory with statuses), PLAN-SUMMARY synced
(the Waves 15/16/17 rows).

> **The 0.27.0 release gate (ADR-0177, №461): 0.27.0 = release of
> freeze-lifting, gated by ADR-0177 criteria. The 0.27.0 tag is NOT
> published until the unfreeze criteria are green.**
...
- **The risk-based review lands (naryad #469, issue #690, Wave 17 — the decision 7-A.2)**: the review goes by RISK, not by order. The checked-in checklist `docs/risk-review-checklist.md` (seven items: the execution context, the result confidentiality labels, the fail-open/fail-closed defaults, the NaN/empty/unbounded values, the TW/VM parity for stateful names, the core joints, the error surface) is enforced by the new `risk-review (non-blocking)` CI job: a PR touching `src/io.rs`, `src/server.rs`, `src/audit.rs`, `src/semantic.rs`, the `llm*` family, or ADDING a `spawn` triggers `scripts/ci/risk_review.py`, which publishes the MECHANICAL surface (checklist item → file:line → observation) as the job summary — the reviewer works through the checklist against that map, the verdict is never the writing agent's self-check. The job is ADVISORY (it never gates a merge) until a separate owner decision — the status is documented in the checklist itself. The boundary rules hold: the scan stays inside the perimeter (plus the spawn-adding files), the job does not replace the completion audit (М3), and a finding becomes a separate naryad, never a silent fix inside the reviewed PR. Functional verification: a synthetic risk diff (a new route with a spawn and fail-open defaults in server.rs) triggers the job and produces the mapped report; a clean diff leaves the run neutral.

- **The type system stage 0 lands (naryad #467, issue #688, Wave 17 — the decision 3-A)**: the signature `Type` enum enters the builtin registry — `Int`, `Float`, `Bool`, `String`, `Unit`, `List`, `Map`, `Struct`, `Fluid`, `Opaque(OpaqueKind)`, `Labeled(Box<Type>, Label)`, `Unknown` — in the new `src/builtins/sig_types.rs` module. The taxonomy is the naryad's fixed minimum: the nine structural kinds mirror `Value`'s shapes, `OpaqueKind` uses the audit §4.4 grouping (Secret/Reflex/Grant/Series/Media/Embodied + Other), `Label` is the Internal/Private pair, and `Unknown` is the honest "not typed yet" — never a fictitious precise type. The conversion of the string path happens AT THE REGISTRY FILL SITE: the `spec!` macro gains typed variants (`spec!(..., handler, "Type")`) whose expansions call the const `Type::from_path` at `BUILTIN_REGISTRY` construction; the flat const parser is byte-table based (str pattern matching is not stable in const fn) and `List<...>` erases to `List` (parametric-free stage 0); the full non-const `parse_type` lifts the labeled path (`X<private>` → `Labeled`) and is pinned to agree with `from_path` on the flat vocabulary. 49 verified spec rows are typed at the start (the string/bool/float/list/unit core — every row verified against its handler's actual `Ok` return; the polymorphic `len`/`length` are honestly `Float`, `reverse`/`redact` stay `Unknown`). The owner's CI-strengthening lands with the rows: the typed-signature share (non-Unknown share of the registry) is printed on every CI run by the new `type-signatures (blocking)` job (`scripts/ci/type_signature_share.py --gate`), the checked-in floor is the start value (49/500 = 9.80%, `scripts/ci/type_signature_baseline.txt`, integer-exact basis points), the floor moves ONLY UP, and the in-tree test `tests/type_signature_metric.rs` double-locks the floor from inside the binary over the live registry. The runtime is untouched: no program changes, no runtime path changes — the field is metadata the compiler and the metric read; the whole lib suite (813 tests) is green unchanged.

- **The reflex transfer group lands (naryad #466, issue #687, Wave 17 — the decision 4-A step 3, group 4: reflex)**: the seven reflex builtin names (`reflex_train`, `reflex_predict`, `reflex_save`, `reflex_load`, `reflex_metrics`, `reflex_list`, `reflex_generate`) move OUT of both backends into the shared live module `src/reflex_ops.rs` — the name literals are replaced by the module's named constants at all 25 dispatch sites (the seven VM `call_reflex_builtin` sites, the seven TW flow-path `invoke` sites, the seven TW expression-path chain sites and the four TW bare-Ident resolution guards), keeping the SAME dispatch order as before. The name literals now live only in the shared module, so the №462 counter drops 42 → 35 (the baseline regenerated in the same PR; the threshold moves ONLY down). Unlike the memory/db/sessions groups, no function bodies moved here: the reflex bodies were already shared since their birth — №179b (train/predict), №180 (save/load), №187 (metrics/list) and №193 (generate) put the whole dispatch logic in `src/builtins/reflex.rs` and both backends delegate to it — the same consent-pair shape group 3 documented, so the transfer is the name constants plus the `handles()` hook. The module is the shared HOME, not a unification: every per-backend marshaling form stays byte-identical to its inline original — the TW flow path takes the registry via `get_mut()` ("reflex registry poisoned") and clones the persist path BEFORE the registry take, the TW expression path locks the Mutex with the per-name texts ("reflex_X: registry lock poisoned") and fetches the persist path AFTER the lock, the VM touches its registry field directly with no lock (train/load `&mut`, the rest `&`), and the bare-Ident first-argument guards resolve a bare model Ident through `reflex_names` into `Value::Reflex` with the per-name "not declared" error texts. The live-contract posture continues the group 1/2 line: the dead `RuntimeContext` stub stays deleted (№465), each backend passes its own registry, names map and persist path explicitly — nothing for this module to own beyond the names. 3 new contract unit tests pin the name contract (the `handles()` coverage, the registry-spelling lock of the seven constants, their distinctness). The fuzzer diff-proof: the divergence CLASS SET is identical before/after the transfer (the №465 pin holds; no reflex class was ever pinned). The media/vision cluster (20 names) transfers with №472; the remaining other-cluster names follow in the next transfer PRs — the issue stays open until the threshold reaches the fact.

- **The sessions transfer group lands (naryad #466, issue #687, Wave 17 — the decision 4-A step 3, group 3: sessions)**: the seven sessions-group builtin names (`conv_start`, `conv_add`, `conv_history`, `conv_context`, `conv_end`, `consent_grant`, `consent_revoke`) move OUT of both backends into the shared live module `src/session_ops.rs` — the five former `Interpreter::invoke_conv_*` bodies, the five inline VM dispatch bodies, and the six consent dispatch name literals keep their exact per-site marshaling hooks (const-name checks in the SAME dispatch order as before). The name literals now live only in the shared module, so the №462 counter drops 49 → 42 (the baseline regenerated in the same PR; the threshold moves ONLY down). The module is the shared HOME, not a unification: the TW `conv_add` lane runs the ADR-0053 auto-compression tail (LLM summarization through the interpreter's SmartRouter, the `messages.len() > compress_after` condition checked AFTER the push and the `last_active` update) while the VM lane has never compressed — the shared body takes the tail as an injected `FnOnce(&mut Conversation)`; the TW site passes its compression callback, the VM site passes a no-op. The consent pair never had duplicated bodies (both backends already delegate to `crate::builtins::consent`) — the transfer there is the name constants plus the `handles()` hook. Every per-backend form is a verbatim transplant, pinned by 19 new contract unit tests over plain stores (the exact error texts both sides, the non-string-text `format!` coercion, the evict-oldest-at-`max_messages` invariant, the tail-observes-post-push order, the `Message` struct shape, the join format, the unconditional "ok" of `conv_end`, the registry-spelling lock of the seven constants). The live-contract posture continues: no hidden global and no revived dead struct — each backend passes its own conversations store and config explicitly, the divergent tail is the mock-tested seam. Mutation verification: flipping the eviction comparison (`>=` → `>`) fails the eviction contract test. The fuzzer diff-proof: the divergence CLASS SET is identical before/after (the №465 pin holds); crosscheck, consent suites, conversation contract and architecture gates stay green. The reflex/media/other groups follow in the next transfer PRs — the issue stays open until the threshold reaches the fact.

- **The db transfer group lands (naryad #466, issue #687, Wave 17 — the decision 4-A step 3, group 2: db)**: the seven db builtin names (`db_execute`, `db_execute_with_grant`, `db_insert`, `query`, `query_param`, `query_row`, `query_scalar`) move OUT of both backends into the shared live module `src/db_ops.rs` — the five former `Interpreter::invoke_*` methods, the inline TW `db_insert` body in execution.rs, and the seven VM dispatch bodies keep their exact per-site marshaling hooks (const-name checks in the SAME dispatch order as before; the subset sites keep their subsets, the `on_deny` sink label and the hooks write-builtin list reference the named constants). The name literals now live only in the shared module, so the №462 counter drops 56 → 49 (the baseline regenerated in the same PR; the threshold moves ONLY down). The module is the shared HOME, not a unification: the TW returns the affected-row STRING from `db_execute` while the VM returns `Unit`; the VM stringifies `query_row` params (the №465-pinned divergence class) while the TW binds typed (the №381 contract); the `db_insert` second-argument error text differs by the shape suffix; the VM lazily opens the connection on first use (№409) — every per-backend form is a verbatim transplant, pinned by 16 new contract unit tests over a `VmDbAccess` mock and in-memory sqlite (the exact error texts both sides, the row shapes, the stringify-vs-typed bind `typeof()` proof, the grant state/TTL/scope gate sequence, the Once-grant consume-only-on-destructive contract). The genuinely shared pieces factored once: the `query_param` parse/return shape (the context accessor injected) and the grant-use note format. The live-contract posture continues: the VM state is reached through the new `VmDbAccess` trait (the lazy-open hook, the connection, the server query params) — the RuntimeContext stub stays deleted, the contract is mock-tested. The fuzzer diff-proof: the divergence CLASS SET is identical before/after the transfer; the crosscheck and the docs gates stay green. The sessions/reflex/media groups follow in the next transfer PRs — the issue stays open until the threshold reaches the fact.

## [0.26.0] - 2026-09-26

- **The lean front door and the release docs (naryad #460, issue #679, audit 25.09 §7.1/§7.2)**: the README is cut from 1139 to 300 lines — the dense sections (overview, advantages, primitives, architecture, features, the full CLI tour, the dev guide, self-hosting/packaging) moved VERBATIM into `docs/book/src/*` pages with SUMMARY entries, every README section carrying a successor anchor link; the contradictory hand-written numbers are gone — the metrics table is GENERATED by `scripts/gen_metrics.py` (version, builtins/modules, SVG builtins, grammar rules, ADRs, example programs, file sizes — computed from the repository state, validated by the new `docs-metrics (blocking)` CI job and independently recomputed by `tests/readme_consistency.rs`); the newcomer contract (the №373/№375/№384-pinned subsections) stays in the README as the 2-minute read. SECURITY.md drops the rotting version table for the versionless formula: security fixes land on the latest minor release only, with the release feed as the authoritative signal. This release 0.26.0 is the security-by-default release — every finding of the audit-25.09 wave (16) closes here.

> The security-by-default release: every finding of the 25.09 audit wave (16) is closed here — the mock-LLM fail-loud default, the file-ingest gate, the strict serve context, the Category-A hardcoded-secret gate, the secret-scanning tripwire, and the release docs themselves (this file, the README, SECURITY.md).

### Wave 16 (dispatch #681 — the full re-audit 25.09 reaction; security by default at the seams)

- **HARDCODED_SECRET is Category A now (naryad #458, issue #677, audit 25.09 finding 3.6 Medium)**: the audit finding — `check_secrets` ran ONLY in `mlog audit` (a warning nobody compiles against), so a pattern with a literal provider token (`"ghp_…"` and friends) COMPILED and RAN — is closed by a split: the HIGH-PRECISION provider prefixes (`ghp_`/`gho_`/`ghs_`/`github_pat_`/`sk-ant-`/`AKIA`/`ASIA`/`xox[bpa]-`/`glpat-`/`AIza`/PEM headers — the existing №102 vocabulary moved as-is, no new formats) are promoted into `audit_category_a` as the Error `HARDCODED_SECRET`, blocking compilation on run/compile/serve/mcp-serve (the same four enforcement points, zero new wiring); the name heuristics (`api_key`, `token=`) and the generic patterns stay WARNING-level `SECRETS` in `mlog audit` — explicitly NOT tightened (the naryad boundary). The string-literal walkers are extracted into ONE shared SSOT (`collect_decl_string_literals`) — the warning-level and the blocking-level checks cannot drift apart in coverage. The escape crane is documented in the `env()` REFERENCE block (both curated and generated rows): read secrets from `env()` or bind them to a `Secret`-typed entity; the EXAMPLE placeholder convention (the AWS docs key `AKIAIOSFODNN7EXAMPLE`, the №459 synthetic-fixture format) keeps fixture data clean, and short prefix strings stay below the same length thresholds the audit always used. The corpus is reconciled: the two examples that carried real-format demo literals (`examples/p102_secret_patterns.mlog` — the №102 audit fixture, and `examples/leak/ok_02_redact_secrets_print.mlog` — the redact demo) now carry EXAMPLE-marked placeholders (their warning-level audit-bait purpose unchanged; the redact-before-sink demo still masks). Tests `tests/naryad_458_hardcoded_secret.rs` (7, BLOCKING): the ghp_ literal refuses `run_program` with the stable code, every provider format blocks, `env()` stays clean, EXAMPLE fixtures stay clean, the name heuristics stay warning-level (compile passes + `mlog audit` still warns), short prefix strings stay clean, and the repository's own example corpus (~260 programs) scans ZERO `HARDCODED_SECRET` on every CI run.

- **The secret-scanning tripwire (naryad #459, issue #678, audit 25.09 findings 3.7 Low + 3.8 Medium)**: the last real token literal from the v0.17 audit (the audit-#5 reproduction fixture in the audit tests and `examples/p102_secret_patterns.mlog`) is replaced with a synthetic of the exact same format (`ghp_` + 36 chars, the `EXAMPLE0000…` convention — same length, so every length-based detection assertion stays green; the AWS fixture moves to the canonical docs key `AKIAIOSFODNN7EXAMPLE`), and `gitleaks` joins CI as the `gitleaks (blocking)` job — a pinned (v8.30.1, checksum-verified) working-tree scan (`--no-git`: the git HISTORY keeps the pre-459 literal by the no-rewrite boundary — the mitigation there is token REVOCATION, the owner's line in the issue, not history surgery) with `.gitleaks.toml` allowing ONLY the sanctioned synthetic shapes (EXAMPLE-marked fixtures, the GitLab fixture body beyond gitleaks' capture group, the №172 test env values, the №274 test-JWT signature, the `[REDACTED:…]` masked placeholders). `cargo audit` re-run WITHOUT `--ignore RUSTSEC-2026-0187` is CLEAN (dependabot's pdf-inspector 1.23.0 + lopdf 0.45.0 resolved the advisory) — the ignore and its comment are removed; the unmaintained-crate warnings stay advisory.

- **The strict serve context (naryad #457, issue #676, audit 25.09 finding 3.4 Medium)**: the serve execution-context default is INVERTED — the permissive-by-default model (every new launch point silently inherited the process context until an engineer remembered the guard) is replaced by strict-by-default: `cmd_serve` calls `set_process_mode(ProcessMode::Serve)` before the routes are built, so every UNMARKED thread resolves to `ExecContext::ServeRoute` and the №253/№259 gates apply; the ONLY exception is the explicit top-level registration zone (`TopLevelRegistrationGuard` wrapped around the declaration-registration loop in `run_server`) — a forgotten mark at a new launch point now costs a spurious loud DENIAL, not silent process rights (the №457 inversion of the failure mode). The audit's concrete hole is closed at both levels: the cron/webhook tick executor `execute_tick_call` (the ONLY launch point still unguarded — TW routes, VM routes and MCP tool methods carried the guard since №253/№536) now sets `ServeRouteExecGuard` as the first line of its spawn_blocking closure, so a tick `env()` refuses with the stable `ENV_NOT_PERMITTED` (identical for existing and non-existing names — the gate is before the read) and a tick `exec()` is gated by the SERVER flag `METALOGOS_SERVE_ALLOW_EXEC` while the process flag `METALOGOS_ALLOW_EXEC=1` does NOT apply (the «замена, не AND» posture verbatim); the №259 allowlist/escape-hatch semantics are unchanged and stay dynamic per call. The full launch-point inventory is re-walked (TW route body, VM route body, MCP tool methods, cron ticks, webhook self-calls, top-level registration — each with its context pinned); `docs/threat-model.md` and `docs/limitations.md` state the new model. Tests `tests/naryad_457_serve_context.rs` (5, BLOCKING): the tick env denial for a SET variable, the tick exec refusal naming the server flag under a set process flag, the allowlist read still working in ticks, the inversion machinery (unmarked = ServeRoute; top-level = Process; the explicit route guard wins inside the zone; exec_gate agreement), and the TW route regression under the inverted default (denied without flags, the escape hatch still working). Mutation ≥2/2 VERIFIED (M1 the tick guard removed — 2 tests kill; M2 the inversion neutralized — the machinery test kills).

- **Mock-LLM fail-loud by default (naryad #454, issue #673, audit 25.09 finding 3.1 High)**: `create_llm_backend()` inverted — `METALOGOS_MOCK_LLM` unset now selects the REAL backend, which fails loudly at the first call without credentials ("Anthropic requires METALOGOS_API_KEY…", no network is touched); the mock is active ONLY when the variable is explicitly `1`/`true` (case-insensitive). `MockLlm` no longer ECHOES the effective prompt (the leak: instructions + recalled memory + conversation history returned to the HTTP client as a "model answer") — it answers with the deterministic non-echo marker `[mock-llm:<8 hex of the prompt hash>]` (stable `DefaultHasher`, same prompt → same marker, input ignored). The mock decision is a single predicate `llm::mock_llm_requested()` consumed by the factory and by all five legacy re-read sites (learnable, conversations, hooks, vm ×2) so the backend choice and the trace/accuracy labeling can never disagree; the media/backend mock seams (`METALOGOS_LLM_MOCK`, ADR-0133/0165) are a separate documented contract, untouched. `mlog serve` announces an active mock twice — the `METALOGOS_MOCK_LLM=1 (mock responses)` danger flag and a loud `[serve] LLM MOCK MODE ACTIVE` line; the obsolete "set METALOGOS_MOCK_LLM=false" hints are reworded. Test harness opts in explicitly: the audit repro `tests/mock_llm_not_default_in_serve.rs` (5 tests: serve default fail-loud with the missing-config named, serve explicit-mock non-echo marker, VM flow-path parity both modes) is BLOCKING; golden examples with LLM blocks carry `examples/X.env` sidecars (`METALOGOS_MOCK_LLM=1`) and their `.expected` files are regenerated under the marker; 11 test files that silently relied on the mock default now opt in explicitly; `run_doc_tests` opts the doc corpus in unless the operator pinned the variable. Living docs (README, REFERENCE, limitations, ADR-0037 status note) state the new default; the `[MOCK: …]` builtin seam and the №334/№407 media mock contracts are unchanged. Found during the repro (pre-existing, reported to the tracker): on the VM serve backend a route body cannot invoke a learnable pattern at all (`Vm::load_program` leaves learnables empty on the serve path) — no LLM call is reachable there; VM parity for №454 is pinned on the flow path.
- **Distillation is gated now (naryad #456, issue #675, Wave 16)**: the №166 guarantee is enforced — the TEACHING→DISTILLED switch requires a deterministic holdout validation: `Dense::train`'s 80/20 holdout accuracy (№179, previously DISCARDED — `let _result`) must be ≥ `distill_min_accuracy` (new learnable field, default 0.85) with a holdout of at least 4 examples (smaller holdouts refuse the switch, loud `[AUDIT] distill.rejected` on every rejection); predictions fail CLOSED on non-finite confidence (the old `partial_cmp(...).unwrap_or(Equal)` silently answered labels through NaN); `fallback_if` defaults to `confidence < 0.7` instead of None (a `distill_to` pattern is no longer barrier-free); `Dense::train` aborts with an error (model not promoted) on a non-finite epoch loss. TW and VM mirror the same gates, the same constants and the same audit wording (parity by construction). Tests: 9 unit (the switch gates + the NaN fail-closed + the confident-answer positive control, both backends) + 4 integration (the grammar field, the optionality, the field-order freedom, the TW/VM parity).



- **The file-ingest gate (naryad #455, issue #674, audit 25.09 finding 3.2 High)**: `read_file` was the live bypass of the №259 env-gate — no serve-route context check, so a route body read `.env` (and the SQLite files, the grammar, the git metadata) straight out of the application directory. Three layers, by the naryad: **(1) the deny-list inside the sandbox, context-independent** — `.env*`, `*.db`, `*.sqlite*`, `.git/**`, `*.mlog`, `metalogos.toml`, `.mlog/**` refuse with the stable `SANDBOX_SENSITIVE_PATH` code (a `try`-branchable typed refusal, the №254 convention), matched on the RAW path form (a policy refusal is loud even when the file is missing) AND on the resolved canonical path (symlink-proof); **(2) the serve-route containment** — in the `ServeRoute` exec context (the SAME thread-local the №253 exec-gate and the №259 env-gate reuse — no second flag-hack) file ingest is allowed only from the data directory (`METALOGOS_DATA_DIR`, default `./data`; the root-resolution failure is fail-closed with the same code); **(3) the escape crane + labels** — `METALOGOS_SENSITIVE_PATH_ALLOWLIST="NAME1,NAME2"` (the №259 allowlist format) unblocks an explicitly named file, and the static label engine marks sensitive-named literal reads SECRET (the audit's `Label::Internal` finding — the stok-checks now refuse such content as private egress); the containment is NOT bypassable by the allowlist. **(4) The taint layer** — a path from an untrusted source (query_param/json_body/form_data/http_get) in a file-ingest position is the category-A `UNTRUSTED_FILE_PATH` (the file-channel twin of UNTRUSTED_EXEC_DECISION; deny-reason vocabulary 13 → 14, the №392 exhaustive-match analyzer enforces the new arm). `read_file_tokens` joins the SAME gate — before the naryad it had NO sandbox at all (any absolute path was readable); its success/error contract is otherwise unchanged. The audit repro `tests/serve_read_file_env_denied.rs` (5 tests, BLOCKING) pins both exploit shapes refusing to START (category A via the real `run_server` path) and the runtime defense-in-depth (a RUNNING server refuses deny-listed and outside-root ingests with the stable code, the marker never reaches the client; data-directory reads keep working); `tests/naryad_455_file_gate.rs` (11 tests) pins the deny-list vocabulary, the allowlist crane, the Secret label, the taint class and the corpus-shape no-false-positive. Mutation ≥2/2 VERIFIED (`scripts/mutation_verify_455.sh`: M1 the deny-list disabled — 6 tests kill; M2 the containment disabled — 1 test kills).

### Wave 17 (gh#695 — the implementation of the 7 strategic decisions of gate gh#680; the freeze lands first)

- **The memory transfer group lands (naryad #466, issue #687, Wave 17 — the decision 4-A step 3, group 1: memory)**: the four memory builtin names (`memorize`, `recall`, `forget`, `recall_top_k`) move OUT of both backends into the shared live module `src/memory_ops.rs` — the two TW dispatch routes (the pattern/route step path and the flow-step expression path carried two identical copies of the four intercepts) and the VM `call_builtin` keep a single `handles()`/`dispatch` marshaling hook each; the name literals now live only in the shared module, so the №462 counter drops 60 → 56 (the baseline regenerated in the same PR; the threshold moves ONLY down). The module is the shared HOME, not a unification: the TW hybrid store lane (FTS5 BM25 + cosine RRF, the knowledge-graph walk, embeddings) and the VM simple-memory twin (substring + activation, token-level AND top-k, case-insensitive forget) stay deliberately separate per the №442 posture — a transfer PR must not change semantics; the per-backend parse shapes (the VM's 0.0 recall threshold vs the TW's 0.3, the `{:?}` vs `type_name()` error texts, the TW case-keeping vs the VM case-folding forget) are preserved verbatim and pinned by 8 new contract unit tests over a `VmMemoryAccess` mock and the in-memory stores. The genuinely shared pieces (the timestamp helper, the JSON result shape of `recall_top_k`, the typed-lane fail-closed gate sequence with the №442 ledger records) are factored once. The dead `RuntimeContext` stub was deleted by №465; the replacing LIVE contract is the `VmMemoryAccess` trait (the store accessors + the recall lane) — the VM fields stay private to the vm module, the contract is unit-tested. The fuzzer diff-proof: the divergence CLASS SET is identical before/after the transfer (the pinned surface unchanged); the crosscheck stays green. The db/sessions/reflex/media groups follow in the next transfer PRs — the issue stays open until the threshold reaches the fact.

- **The TW/VM diff fuzzer (naryad #465, issue #686, Wave 17 — the decision 4-A step 2, strictly before the #466 transfer)**: the audit's 4.1 finding (the crosscheck is blind to divergences by construction — a fixed corpus both backends can execute) is closed by a seeded, deterministic program generator (150 programs per CI run + a checked-in seed corpus) driving BOTH backends through the same in-process harness and comparing outcome + stdout + the error class; found divergences are minimized (statement-level delta debugging) and ratcheted BY CLASS (`tests/fuzz_corpus/known_divergences.txt` — a new class fails CI loudly, a known class is reported honestly; the corpus is NOT tuned to green). The first honest run (150 + 5 seeds): 152 divergences collapsing into 11 classes, headlined by TWO real data points for the #466 prioritization — `memory_forget` is MISSING from the VM dispatch entirely (TW runs it, the VM refuses at compile; the memory group stays FIRST), and an entity referenced inside a pattern body resolves in the VM (compile-time constant) but is `undefined variable` in TW (the program-context semantics diverge — the owner gates the fix). The dead `RuntimeContext` stub (one `#[allow(dead_code)]` field, zero consumers since №178) is DELETED by the naryad's explicit-decision clause: the LIVE RuntimeContext is designed and tested by №466 as part of the transfer, not kept as dead weight.

- **ADR-0178: the generative contour boundary (naryad #464, issue #685, Wave 17 — the decision 5-B vehicle)**: the audit's 5.2 finding (the №166 boundary silently revised without an ADR) is closed by policy + mechanism — the in-tree CPU generative contour is declared EXPERIMENTAL, bounded to an explicit workload list (on-device classification, adequacy/quality scoring, small transformation tasks; prose generation is explicitly NOT a target — the vision's doubt 2 answered by fixing the workload, not by abstraction), executably frozen by the №463 CI stop-list (any growth red; the baseline moves only down; manifest changes REQUIRE an ADR patch in the same PR), exit-ready (the `reflex-gen` home after the reflex crate extraction, №472's roadmap, 0.27+), and the expansion preconditions are enumerated (№456/№457 closed — landed; background training out of the mutex, the runtime Secret check in the embedding, the call budget — tracked, blocking expansion only). The removal/expansion rights belong to the OWNER via an ADR patch — the prose and the gate say the same thing. `docs/limitations.md` carries the honest "Generative contour (experimental)" section; the ADR index regenerated (170 entries).
...
- **The plan-v2 memory contract (naryad #453, issue #662)**: ADR-0176 ACCEPTED — the owner chose **Variant B** (the minimal contract): the six-point SSOT of the shipped memory surface (the typed `Memory<K>` lane as the single SSOT; consent fail-closed on every read/export surface; the derived→poisoned cascade with the retained VETO; TTL auto-forget + the ACT-R decay/boost ranking; `[MEM]` provenance + the `memory.*` ledger family; the FTS5/InMemory engines declared swappable storage details), with the points-to/static poison lattice (the P2 park), `find`/`inspect` (named stubs) and the real-weights lines deliberately outside; the Memory boundaries row in limitations cites the contract; the REALITY §6.9 named gap closes at the next recount (Wave 16).

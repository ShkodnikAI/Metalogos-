...
- **Kitchen-camera e2e: the sealed-egress backstop joins the deny-event layer and the signed ledger (naryad #397, acceptance item 1 of the wave-3 dispatch)**: the canon §5.3 scenario now runs END-TO-END over the wave-3 runtime stack — the static half (a DECLARED-private camera is refused at compile time, `SECRET_LEAK`) was always covered; the RUNTIME half is new. A runtime-sealed frame (the origin is declared public, the frame's sensitivity is decided at runtime — knowledge the static layer cannot have) refuses materialization inside `media_save` ([`MEDIA_SEALED_EGRESS`], the №331 backstop), and since this change that refusal is a typed №392 deny event (reason `MEDIA_SEALED_EGRESS`, class `file`): `on_deny(file/*)` handles it (degraded Unit), the ledger journals it as a side effect of the refusal path itself (ADR-0167 §3.4), and with no covering handler the loud coded error is unchanged (the №392 posture). The deny-reason dictionary extends 13→14 (`src/deny.rs` `DENY_REASONS` — the extending-classes group, same gate dictionary); the exhaustive-matching examples/tests follow (`w2_deny_exhaustive` gains the 14th arm, `w2_deny_incomplete.error` lists the new reason). Wiring: TW both dispatch sites (`src/interpreter/execution.rs` invoke + expression path) and VM both `CallBuiltin` arms (`src/vm.rs`), following the GRANT_* wire pattern verbatim; the event's `argument` carries the materialization PATH (the forensic answer to "where would the bytes have gone"), the `label` carries the entry's declared sensitivity parsed from the refusal (helpers `deny::sealed_egress_argument` / `deny::media_seal_sensitivity`, unit-tested). The e2e example `examples/w3_kitchen_camera.mlog` (+ golden): capture → runtime seal → THE DENY (explainable, journaled, degraded) → the sanctioned degraded path (the frame stays sealed in the store) → the office-shaped granted destructive DELETE on the motion-event night log (N(1) quota; the second granted call refuses GRANT_EXHAUSTED → `deny.IRREVERSIBLE_NO_GRANT`) → key rotation + snapshot + EXPORT; `mlog ledger verify target/w3_kitchen_camera_export.jsonl` validates the signed chain externally (pure hash + Ed25519). Evidence: `tests/naryad_397_kitchen_camera_e2e.rs` (5 tests — TW/VM parity on the golden, external verification + the deny record triple `runtime`/`file`/args_hash in the exported chain, the loud no-handler refusal, the static gate unchanged). REFERENCE `media_save` row updated. **Latent №393 bug fixed en route**: `append_record` numbered the first record of a fresh chain `seq = head_seq + 1` (head_seq initialized 0 = genesis) while `verify_records` enforces the ADR-0167 §3.2 start rule "genesis (seq 0)" — a fresh chain's export could NEVER pass external verification (`record 1: chain must start at seq 0 …, got seq 1`); the genesis position is disambiguated by the genesis prev_hash (empty chain → seq 0; a chain whose last record is seq 0 carries that hash as the head), and the e2e's external-verify assertion is the regression pin.

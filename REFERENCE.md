...
| `media_save(...)` | 2..3 | — | `media_save(handle, path)` — the ONLY sanctioned media materialization: writes the exact bytes to a sandboxed file (№131/№252). Sink: №325 clearance at compile time (SECRET_LEAK for private labels) + runtime backstop MEDIA_SEALED_EGRESS for sealed entries — since №397 the backstop refusal is a typed deny event (reason `MEDIA_SEALED_EGRESS`, class `file`): `on_deny(file/*)` handles it, the ledger journals it, no handler keeps the loud coded error. Unsealed ONLY by the №387 LikenessToken credential. Returns the path. |

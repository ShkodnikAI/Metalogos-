...
> **Coverage note (v0.19):** This section documents **~59%** of the 389 registered builtins (230 of 389).
...
### 4.22. Vision — provenance, персистенция и безопасная загрузка весов (наряды №210–№243, ADR-0122/0124/0125)
...
Здесь документированы **реальные** встроенные функции provenance/персистенции/загрузки. Семейство `vision_generate`/`vision_edit`/`vision_list`/`vision_export`/`vision_export_raw`/`vision_save`/`vision_load`/`vision_fetch_weights` перехватывается (или диспетчеризуется) интерпретатором/VM (состояние реестра и/или соединение с БД программы) и описано в ADR-0122/0124; `vision_edit` — реальный путь in-context редактирования с №243 (R6.2); `vision_save`/`vision_load` — реальные пути SQLite-персистенции с №242 (R6.1).

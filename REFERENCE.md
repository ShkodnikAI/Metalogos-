...
### 4.22. Vision — provenance, persistence, and safe weight loading (naryads #210-#243, ADR-0122/0124/0125)
...
This section documents the **real** built-in provenance/persistence/loading functions. The `vision_generate`/`vision_edit`/`vision_list`/`vision_export`/`vision_export_raw`/`vision_save`/`vision_load`/`vision_fetch_weights` family is intercepted (or dispatched) by the interpreter/VM (registry state and/or a connection to the program's DB) and is described in ADR-0122/0124; `vision_edit` is the real in-context editing path since #243 (R6.2); `vision_save`/`vision_load` are the real SQLite-persistence paths since #242 (R6.1).

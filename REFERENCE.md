...
### 4.22. Vision — provenance, персистенция, LoRA-адаптеры и безопасная загрузка весов (наряды №210–№244, ADR-0122/0124/0125)
...
Здесь документированы **реальные** встроенные функции provenance/персистенции/загрузки. Семейство `vision_generate`/`vision_edit`/`vision_list`/`vision_export`/`vision_export_raw`/`vision_save`/`vision_load`/`vision_lora_load`/`vision_lora_generate`/`vision_fetch_weights` перехватывается (или диспетчеризуется) интерпретатором/VM (состояние реестра и/или соединение с БД программы) и описано в ADR-0122/0124/0125; `vision_edit` — реальный путь in-context редактирования с №243 (R6.2); `vision_save`/`vision_load` — реальные пути SQLite-персистенции с №242 (R6.1); `vision_lora_load`/`vision_lora_generate` — реальные пути LoRA-адаптеров с №244 (R6.3; семейство достигло потолка ADR-0124 §3 — 10 функций).

**Композит-семантика `model_sha256` при применённом LoRA-адаптере (№244, Block 2.4)**: на пути `vision_lora_generate` поле манифеста несёт составной отпечаток `sha256("{base}\nlora:{name}:{lora_sha256}")`, где `base` — обычный отпечаток дерева весов (`weights_tree_sha256`, в т.ч. честный маркер «unpinned» — композит честен и над ним), `name` — персистентное имя адаптера из БД, `lora_sha256` — SHA-256 байтов адаптера из БД (тот же, что в integrity-пине). `model_id` и watermark остаются БАЗОВОЙ моделью — адаптер это дельта, а не модель; формула зафиксирована в `vision_lora_composite_model_sha256` и зеркалится в doc-comment `VisionManifest::model_sha256` (7 полей манифеста НЕ расширяются).

| Функция | Сигнатура | Возвращает | Описание |
|---|---|---|---|
| `vision_edit(handle, prompt)` | `(Vision, String) -> Vision` | `Vision` (новый хэндл) | **In-context редактирование подписанного артефакта** (№243, R6.2). Источник ОБЯЗАН быть подписан: артефакт без манифеста провенанса = громкий отказ (нечего честно унаследовать; производить unsigned через реальный compute-путь запрещено ADR-0125) — raw-экспорт (`vision_export_raw`) не затронут. Пайплайн: source-PNG → декодация → громкий dims-контракт (R4.1: 256..=4096, ×16; кратность VAE-фактору — тихий ресайз запрещён, выход сохраняет разрешение источника) → VAE-энкодер (не-decoder префиксы того же pinned VAE-файла; encode в posterior MODE) → референс-латент → Qwen3-4B на edit-промпт → Z-Image DiT с in-context конкатенацией токенов (шумовая ветвь + референс на каждом шаге, `euler_step` только по шумовой) → `EDIT_STEPS = 8` (дистиллированный NFE turbo) → декод → PNG. Выход подписан ВСЕГДА: watermark + манифест, где `model_id`/`policy`/`seed` наследуются от источника, `prompt_sha256` — хэш edit-промпта, `timestamp`/`png_sha256` свежие. Громкие отказы: unsigned-источник, unknown handle, неверный тип, пустой промпт, размеры вне R4.1/не кратны фактору, нет `MLOG_VISION_WEIGHTS_DIR` (переменная и способ включения называются дословно). UserInput-taint промпта — audit-Warning `VISION_PROMPT_USER_INPUT` (тот же check-id, что у `vision_generate`). |
...
| `vision_lora_load(name, path)` | `(String, String) -> String` | `String` (имя) | **Загрузка LoRA-адаптера в SQLite BLOB** (№244, R6.3; ADR-0124 §6 — адаптер живёт ТОЛЬКО в БД программы, сессионного состояния нет). Читает safetensors-файл ОДИН раз — только внутри `MLOG_VISION_WEIGHTS_DIR` (относительный путь, без `..`, расширение `.safetensors`, файл существует; контракт-функция `vision_lora_check_adapter_path`), валидирует громко (обе канонические формы имён diffusers-PEFT/ComfyUI; rank = средняя размерность, scale = alpha/rank или громкий дефолт 1.0; не-F32 апкаст громко; цели — только attention-проекции `to_q/to_k/to_v/to_out.0` по `zimage_expected_keys`; half-pair/не-attention/неизвестный префикс/осиротевшие ключи/dims-mismatch = громкий Err с ПОЛНЫМ перечнем) и сохраняет байты + метаданные (`LoraMeta`: sha256/rank/alpha/scale/targets) в таблицу `vision_lora_adapters`. Порядок проверок предписан: арность → no-db → env → путь → feature-гейт → чтение/разбор → запись; коллизия имени = громкий Err (no upsert). Возвращает персистентный ключ `name`. |
| `vision_lora_generate(decl_name, prompt, lora_name)` | `(String, String, String) -> Vision` | `Vision` (новый хэндл) | **Генерация с применённым LoRA-адаптером** (№244, R6.3). Тот же пайплайн, что `vision_generate`, но DiT строится через `from_weights_with_lora`: `W' = W + scale·(up@down)` в F32 по attention-проекциям; адаптер резолвится из БД на каждый вызов с **integrity-пином** (`sha256(bytes) ≠ meta.sha256` = громкий Err ДО любого compute). Provenance: композит `model_sha256` (см. выше), `model_id`/watermark/policy/seed/steps — из базовой декларации; sign ALWAYS. Громкие отказы: арность/типы, пустой промпт, неизвестный decl (с перечнем), unknown model, нет БД, неизвестный `lora_name` (с `lora_list`), битый meta-JSON, integrity-промах, нет env/компонента, не-1024. UserInput-taint промпта — audit-Warning `VISION_PROMPT_USER_INPUT` (тот же check-id; arg-0/arg-2 не флагаются). |
...
**Пример** (загрузка разрешённого пакета весов и генерация):

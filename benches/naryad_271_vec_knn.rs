...
//
// Без feature `vec` файл компилируется в пустой бинарник-заглушку (harness=false
// bench — это бинарник: без main был бы E0601 на clippy --all-targets и
// vision-tests, что и поймал CI первого прогона спайка). В main НЕ мержится.
...
fn main() {
    bench::benches();
    // criterion_main! разворачивается в fn main на уровне крейта — вызываем
    // группы и final_summary вручную, чтобы main был именно в crate root
    // (внутри mod bench его E0601 не видит).
...
mod bench {
    ...
    fn bench_scale(c: &mut Criterion, n: usize, group_prefix: &str) {
        ...
        eprintln!("[naryad-271] top-1 verified at n={n}: bf={bf_best:.6}, vec0={knn_best:.6}");

...
`Reflex` pillar (naryads №177–196). `builtin_reflex_*_stub` functions
...
**This is not a new problem.** `memorize`/`recall` (naryad №67/72) hit
...
duration of naryads №67 onward.

## Decision

Follow the naryad №67/72 pattern exactly, not a new architecture:
...
(and BPE variants once naryad №195 lands) — replacing the `_stub`
...
## Staging — mirrors how `Reflex` itself was built, not one naryad

| Stage | Scope | Naryad |
|---|---|---|
| 1 | `Vm.reflex_registry` field, `reflex` (classification) declaration + `reflex_train`/`reflex_predict` compiled and dispatched | №199 |
| 2 | `reflex_save`/`reflex_load` (persistence, naryad №180) | №200 |
| 3 | `reflex_seq` (attention/GQA/transformer stack, naryads №183–192) | №201 |
| 4 | `reflex_gen` (generation, KV-cache, naryad №193) | №202 |
| 5 | Tokenization builtins (naryad №194, and BPE from №195 once it lands) | №203 |
...
- `ADR-0105`'s general reasoning (no confirmed case, FOSVED runs on
  ...
  (`match` expression, naryad №118's collection-utils case) — this ADR
  narrows the exception to `Reflex` specifically, not a blanket
  reversal.
- `candle`'s determinism (naryad №176's finding, load-bearing since
  naryad №177) must hold identically when invoked from VM's own
...
- Six naryads, not one — matches the actual scope, avoids the naryad

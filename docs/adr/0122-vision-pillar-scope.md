...
| №215 | R6 | vision_edit, LoRA-адаптеры, vision_save/load (SQLite BLOB) | закрыт целиком нарядами №242–244 (R6 разрезан громко: №242 = save/load, №243 = vision_edit, №244 = LoRA) — truth-up №244 |
| №216 | R7 (опц.) | LoRA-дообучение через candle autograd | reserved (ТРЕНИРОВКА вне скоупа навсегда для R6 — №244 загружает и применяет адаптеры, но не обучает) |
...
| №243 | R6.2 | vision_edit — in-context editing (вторая треть R6; №244 = LoRA). **Block 1**: `VaeEncoder` (`src/vision/vae.rs`, зеркально декодеру) — не-decoder префиксы ТОГО ЖЕ pinned `vae/diffusion_pytorch_model.safetensors` (манифест НЕ расширяется; арифметика 244 = 138 decoder + 106 encoder → quant_conv опционален: потребление/отсутствие = громкая нотка, фактический список = сверка по заголовку при PARKED-прогоне, расхождение = громкий Err с перечнем); `new_tiny` (свои PRNG-слоты) для tiny-контрактов; `encode(img [-1..1]) → [1, C, H/f, W/f]` в posterior MODE (детерминизм Block 2.3), ритуал — алгебраическая инверсия декодер-направления №232; `decode_png` — декод-половина encode_png. In-context edit-цикл: `flow_match_euler_edit` (`sampler.rs`) + `forward_edit` (`dit.rs`, аддитивно) — референс-латент конкатенируется токенами с шумовым на КАЖДОМ шаге (тот же x_embedder + noise_refiner; RoPE t-слоты: шум = cap_len+1 как в generate, референс = cap_len+2 — громкий чек против axes_lens), euler_step только по шумовой ветви, референс чистый; `EDIT_STEPS=8` — дистиллированный NFE turbo (громкая константа с пиннинг-тестом); CFG отсутствует (turbo). **Block 1.3 bit-exact**: wedge-голдены №212 зелёные БЕЗ правок. **Block 2**: `vision_edit(handle, prompt) -> Vision` — источник ОБЯЗАН быть подписан (`manifest: None` = громкий Err ДО env-чека; unsigned остаются рабочими в `vision_export_raw`); re-check model ∈ KNOWN_VISION_MODELS по манифесту источника; наследование model_id/policy/seed + model_sha256 текущего прогона + prompt_sha256 edit-промпта + свежие timestamp/png_sha256 (sign ALWAYS, watermark после — png_sha256 описывает отгруженные байты); dims-контракт: R4.1 (256..=4096, ×16) + VAE-фактор — громкие Err, тихий ресайз запрещён; env/feature-гейты как у generate; перехваты interpreter (eval+invoke) и VM; last-resort стаб truth-up («214/215» → №243 — последний «214/215» покинул репо); naryad_210 truth-up (10→11: −1 стаб-тест, +2 real-path). **Block 3**: taint arg-1 → `VISION_PROMPT_USER_INPUT` (тот же check-id/категория, arg-0 handle не флагается; новых check-id нет). **Block 4**: 14 tiny-контрактов (naryad_243: источник/промпт-зависимость, watermark+7 полей, unsigned-отказ, dims, EDIT_STEPS pin; лекало wedge — синтетика, без сети/весов/#[ignore]) + env-gated `mlog_vision_edit_export_e2e` (loud-SKIP, цель runbook §3.1); runbook §3.1 edit-e2e; без LoRA/гейтов №241/weights.rs/grammar.pest/goldens; реестр 389 не меняется | PR #241 |
| №244 | R6.3 | LoRA-адаптеры — финальная треть R6 «Edit + LoRA»: `src/vision/lora.rs` (разбор safetensors-байтов, ОБЕ канонические формы имён diffusers-PEFT/ComfyUI + `<target>.alpha`, rank = средняя размерность, scale = alpha/rank / громкий дефолт 1.0, громкий не-F32 апкаст в F32, валидация целей: после снятия суффикса только attention-проекции `to_q/to_k/to_v/to_out.0` по `zimage_expected_keys`; half-pair/не-attention/неизвестный префикс/осиротевшие ключи/dims-mismatch = громкий Err с ПОЛНЫМ перечнем); слияние `ZImageTransformer::from_weights_with_lora` (база через нетронутый `from_weights` + `merge_lora_in_place`: W' = W + scale·(up@down) в F32 с возвратом к dtype базы, детерминированный порядок целей, отсутствующая база/выход за n_layers = громкий Err; identity: нулевой up ИЛИ down → выход байт-в-байт базовый); таблица `vision_lora_adapters` (name PK / bytes BLOB / meta_json NOT NULL / saved_at — ADR-0124 §6 SQLite-BLOB, единственный дом адаптера, сессионного состояния НЕТ); `vision_lora_load(name, path) -> String` — чтение ТОЛЬКО внутри weights_dir (относительный путь без `..`, .safetensors, файл существует; контракт-функция `vision_lora_check_adapter_path`), предписанный порядок громких проверок; `vision_lora_generate(decl, prompt, lora_name) -> Vision` — резолв адаптера из БД с integrity `sha256(bytes) ≠ meta.sha256 = громкий Err`, композит-provenance `model_sha256 = sha256("{base}\nlora:{name}:{sha}")` (честен и над «unpinned»), watermark = базовая модель, sign ALWAYS; перехваты ×3, стабы-лекало, taint arg-1 (тот же check-id). Реестр 389→391 — **семейство vision достигло потолка ADR-0124 §3 (10 = верх границы ~8–10): следующий vision-builtin требует правки ADR-0124**. Wedge-голдены №212 bit-exact без правок; weights.rs/grammar.pest/KNOWN_VISION_MODELS/VisionRegistry/ADR-0124/манифест весов не тронуты | PR #242 |

...
1. ~~Golden SHA-256 записи не запинены (Block 2.4 не выполнен) — тест проверяет
   ...
   записям.~~ **ПОГАШЕНО (№230, PR #225)**: `GOLDEN_HASH_P1/P2/P3`
   + `GOLDEN_ANCHOR_BITS_P1/P2/P3` (4 угловых `f32::to_bits()` на промпт —
   integer-exact, иммунитет к print-drift) вшиты в
   `tests/naryad_211_text_encoder_golden.rs`. Тест 1 ассертит хэш + якоря
   бит-в-бит. Запинено по 3 идентичным локальным прогонам (2026-09-08).
2. ~~Локальная копия `generate_uniform_f32` расходится с SSOT-контрактом
   ...
   f32 vs f64 → другие потоки значений при том же seed).~~ **ПОГАШЕНО (№230)**:
   локальная копия удалена; импорт `use crate::nn::attention::generate_uniform_f32;`
   (SSOT). Для корректной работы импорта исправлена импликация feature-флагов:
   `vision = ["candle"]` (раньше `["dep:candle-core", "dep:candle-nn"]` — без
   включения самого `candle` feature-флага, поэтому `#[cfg(feature = "candle")]`
   модули `src/nn/` не компилировались под `--features vision`).
3. ~~Перекрытие seed-потоков: `layer_seed + offset` шарит офсеты между слоями
   ...
   **ПОГАШЕНО (№230)**: `param_seed(master, layer, param)` — splitmix64 finalizer
   над `(master_seed, layer, param)`. `PARAM_EMBEDDING=0` через `PARAM_DOWN=7`
   — фиксированные константы (не перенумеровывать: derivation — часть golden-
   контракта). Тест `param_seed_derivation_is_pairwise_distinct` проверяет
   32 сида (4 слоя × 8 параметров) попарно различимы + не-identity.

Порядок в №230 соблюдён: сначала PRNG SSOT + stream-гигиена (Block 1, меняет
все значения), затем пиннинг golden-записей (Block 2, один раз, по финальным
значениям после 3 бит-в-бит прогонов).

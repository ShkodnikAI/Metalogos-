...
- №243 (PR #241): R6.2 vision_edit — in-context editing (вторая треть R6). `VaeEncoder` в `src/vision/vae.rs` (не-decoder префиксы ТОГО ЖЕ pinned VAE-файла: 244 = 138 decoder + 106 encoder — quant_conv опционален, отсутствие/наличие = громкая нотка, фактический список сверяется по заголовку при PARKED-прогоне; new_tiny для tiny-контрактов; encode в posterior MODE = детерминизм); in-context edit-цикл `flow_match_euler_edit` (EDIT_STEPS=8 — дистиллированный NFE turbo, громкая константа; `forward_edit` в dit.rs — аддитивно: референс-токены через тот же x_embedder + noise_refiner, собственный RoPE t-слот cap_len+2, euler_step только по шумовой ветви; CFG отсутствует — turbo); **wedge-голдены bit-exact без правок** (85ef6a87/860c85b3 — задокументировано в PR). Диспетч `vision_edit(handle, prompt)`: источник ОБЯЗАН быть подписан (manifest: None = громкий Err ДО env-чека; unsigned остаются рабочими в vision_export_raw), наследование model_id/policy/seed + свежие timestamp/prompt_sha256/png_sha256 (sign ALWAYS), dims-контракт (R4.1 256..=4096 ×16 + VAE-фактор — громко, тихий ресайз запрещён), taint arg-1 → VISION_PROMPT_USER_INPUT (тот же check-id, новых нет). Перехваты interpreter (eval+invoke) и VM; last-resort стаб truth-up (последний «214/215» покинул репо); naryad_210 truth-up (10→11 тестов). Runbook §3.1 — edit-e2e для PARKED-прогона. Без сети и весов.
- №244 (PR #242): R6.3 LoRA-адаптеры — финальная треть R6 (№215 закрыт целиком №242–244). `src/vision/lora.rs` (обе канонические формы имён diffusers-PEFT/ComfyUI + alpha; rank = средняя размерность, scale = alpha/rank / громкий дефолт 1.0; громкий F32-апкаст; цели — только attention-проекции по `zimage_expected_keys`; все негативы = громкий Err с полным перечнем); `from_weights_with_lora`/`merge_lora_in_place` (W' = W + scale·(up@down) в F32, детерминированный порядок, отсутствие базы/выход за n_layers = громкий Err; identity нулевого адаптера байт-в-байт — wedge-голдены 85ef6a87/860c85b3 зелёные без правок); таблица `vision_lora_adapters` (SQLite BLOB — единственный дом адаптера, ADR-0124 §6); `vision_lora_load` (чтение только внутри weights_dir, предписанный порядок громких проверок) + `vision_lora_generate` (integrity sha при каждом резолве из БД, композит-provenance `sha256("{base}\nlora:{name}:{sha}")` — честен и над «unpinned», watermark = базовая модель, sign ALWAYS); перехваты ×3, taint arg-1 (тот же check-id), стабы-лекало. **Реестр 389→391 — семейство vision достигло потолка ADR-0124 §3 (10/10): следующий vision-builtin требует правки ADR-0124** (громко в CHANGELOG). Runbook §3.2 — lora-e2e для PARKED-прогона (адаптер кладёт владелец под weights_dir/lora/…). Тесты: 21 tiny-контракт naryad_244 + 3 taint (naryad_240) + 2 стаб-группа (naryad_210, 11→13) + 4 юнита слияния (dit.rs/lora.rs); blob строится candle-райтером (API доступен — отклонений нет). Без сети и весов.

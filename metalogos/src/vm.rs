...
pub struct Vm {
    ...
    /// Optional JIT compiler for hot-path native code generation.
    jit: Option<JitCompiler>,
    /// Per-pattern call counters for hot-path detection.
    call_counts: Vec<usize>,
    /// Hot-path threshold: when a pattern's call count exceeds this,
    /// it becomes a candidate for JIT compilation.
    jit_threshold: usize,
}
...
impl Vm {
    ...
    /// Create a new VM with JIT compilation enabled.
    pub fn with_jit(threshold: usize) -> Result<Self, String> {
        let jit_compiler = JitCompiler::new(threshold)?;
        Ok(Vm {
            globals: Vec::new(),
            global_names: Vec::new(),
            patterns: Vec::new(),
            ...
            builtin_names: vec![
                ...
                "recall".to_string(),
                "stdin".to_string(),
                "split_tokens".to_string(),
                "if_eq".to_string(),
                "newline".to_string(),
                "is_string_token".to_string(),
            ],
            memory: Vec::new(),
            relations: Vec::new(),
            rules: Vec::new(),
            mutate_log: Vec::new(),
            collections_loaded: false,
            jit: Some(jit_compiler),
            call_counts: Vec::new(),
            jit_threshold: threshold,
        })
    }

    /// Get the number of JIT-compiled patterns (for introspection/testing).
    pub fn jit_compiled_count(&self) -> usize {
        self.jit.as_ref().map(|j| j.compiled_count()).unwrap_or(0)
    }
    ...
    pub fn run(&mut self, program: Program) -> Result<Option<String>, String> {
        ...
        // Initialize call counters for all patterns
        self.call_counts = vec![0; program.patterns.len()];
        ...
        while ip < code.len() {
            ...
            match instr {
                ...
                Instruction::RegisterPattern(fn_def) => {
                    let _idx = self.patterns.len();
                    // Ensure call_counts grows with patterns
                    if self.jit.is_some() {
                        self.call_counts.push(0);
                    }
                    // Store in a temporary patterns list — we use the index from compile
                ...
                Instruction::CallPattern(idx, arity) => {
                    ...
                    // ── JIT fast path ───────────────────────────
                    // Check if this pattern has been JIT-compiled
                    if let Some(ref jit) = self.jit {
                        if let Some(fn_ptr) = jit.get_compiled(*idx) {
                            // Extract Float arguments from the stack
                            let mut args_f64 = Vec::with_capacity(*arity);
                            for _ in 0..*arity {
                                let val = stack.pop().unwrap_or(Value::Unit);
                                args_f64.insert(0, val.as_float().unwrap_or(0.0));
                            }
                            // Call native code directly
                            let result_f64 = JitCompiler::call_jit(fn_ptr, &args_f64);
                            stack.push(Value::Float(result_f64));
                            ip += 1;
                            continue;
                        }
                    }
                    ...
                    // ── Hot-path detection ──────────────────────
                    if self.jit.is_some() {
                        if let Some(count) = self.call_counts.get_mut(*idx) {
                            *count += 1;
                            if *count >= self.jit_threshold {
                                // Try to JIT-compile this pattern
                                if let Some(ref mut jit) = self.jit {
                                    if jit.get_compiled(*idx).is_none()
                                        && JitCompiler::is_jit_eligible(&pattern)
                                    {
                                        let _ = jit.compile_and_store(*idx, &pattern);
                                        // If compilation succeeded, the next call
                                        // will use the fast path above.
                                    }
                                }
                            }
                        }
    ...
    /// Includes JIT hot-path detection and dispatch for pure Float patterns.
    fn invoke_step(&mut self, name: &str, args: Vec<Value>) -> Result<Value, String> {
        // Check learnables first (never JIT-compiled)
        ...
        // Check patterns
        for (pidx, pattern) in self.patterns.iter().enumerate() {
            if pattern.name == name {
                ...
                // ── JIT fast path for flow pipeline calls ───────
                if self.jit.is_some() {
                    // Increment call count
                    if let Some(count) = self.call_counts.get_mut(pidx) {
                        *count += 1;

                        // Check if JIT-compiled version exists
                        if let Some(ref mut jit) = self.jit {
                            if let Some(fn_ptr) = jit.get_compiled(pidx) {
                                // Extract Float arguments
                                let f64_args: Vec<f64> = args.iter()
                                    .map(|v| v.as_float().unwrap_or(0.0))
                                    .collect();
                                let result_f64 = JitCompiler::call_jit(fn_ptr, &f64_args);
                                return Ok(Value::Float(result_f64));
                            }

                            // Check threshold — compile if hot
                            if *count >= self.jit_threshold {
                                if jit.get_compiled(pidx).is_none()
                                    && JitCompiler::is_jit_eligible(pattern)
                                {
                                    let _ = jit.compile_and_store(pidx, pattern);
                                    // If compilation succeeded, the next call
                                    // will use the JIT fast path.
                                }
                            }
                        }
                    }

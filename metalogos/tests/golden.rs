...
/// Execute a .mlog source with the given base directory and return stdout as a string.
fn run_mlog_with_dir(source: &str, base_dir: &Path) -> Result<String, String> {
    let declarations = metalogos::run_program_with_dir(source, base_dir.to_path_buf())?;
...
fn all_golden_tests_pass() {
    ...
    for (mlog_path, expected_path) in &pairs {
        ...
        let base_dir = examples_dir.parent().unwrap_or(&examples_dir).to_path_buf();
        let actual = run_mlog_with_dir(&source, &base_dir).expect(&format!("execution failed for {:?}", mlog_path));

...
#      docs/research/bench-tree.log (the report's tree form).
#
# Usage:
...
: "${BENCH_NODE:?BENCH_NODE must name this run's tree node (rule 5)}"
...
cd "$REPO_ROOT"
LOG="docs/research/bench-tree.log"
...
path, divisor = sys.argv[1], int(sys.argv[2])
raw = {}
with open(path) as f:
    for line in f:
        line = line.strip()

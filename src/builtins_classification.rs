...
pub static BUILTIN_CLASSES: &[BuiltClassEntry] = &[
    ...
    BuiltClassEntry { name: "recall_top_k", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Pure, rationale: "reads the backend-local memory store (top-k search; Bug #530 — registered so the VM compiles the name both backends intercept)" } },
    ...
    BuiltClassEntry { name: "recall", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Pure, rationale: "real handler (№442) — the front door of memory: the store lane (hybrid BM25+vector on the TW; the VM-native twin) plus the typed lane with consent-gated private containers (fail-closed MEMORY_RECALL_CONSENT_REQUIRED — the refusal is a memory.recall.denied record); typed hits carry the [MEM] provenance suffix; records memory.recall {query hash, containers, hits, consent fact}" } },
    BuiltClassEntry { name: "memory_open", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "returns the Memory<K> container handle — the ingress of the typed-memory surface; a private open is consent-gated INSIDE (active consent for memory:<subject>, №335 — the db_execute_with_grant capability precedent); records memory.open (№350)" } },
    BuiltClassEntry { name: "memory_put", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    BuiltClassEntry { name: "memory_read", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "THE AUDITED READ SINK: public returns String, private returns Secret — print refuses it and redact() (№326/ADR-0136) is the only egress; records memory.read (№350)" } },
    BuiltClassEntry { name: "memory_keys", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Pure, rationale: "lists the container's keys (metadata only; audited) (№350)" } },
    BuiltClassEntry { name: "memory_provenance", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Pure, rationale: "reads the derived-from parent keys of one entry — the raw material of the №351 derived graph; records memory.provenance (№350)" } },
    BuiltClassEntry { name: "memory_export", class: BuiltClass { role: Role::Sink, default_label: Label::Internal, reversibility: Reversibility::Irreversible, rationale: "file-egress sink through the io sandbox — private entries REFUSE (MEMORY_REDACT_REQUIRED: №326 is the only private egress path); records memory.export (№350)" } },
    BuiltClassEntry { name: "memory_cascade_preview", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Pure, rationale: "computes the cascade plan READ-ONLY ({closure, blocked_by}) — the №280 dry-run discipline: preview before any grant is touched; when blocked_by is empty the would-delete set IS the closure; records memory.cascade_preview (№351/ADR-0173 §3.2)" } },
    BuiltClassEntry { name: "memory_retain", class: BuiltClass { role: Role::Sink, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "pins the descendant closure of the key (the CASCADE retain) against cascading forgetting — a retained node inside a forget closure VETOES the whole forget (fail-closed, ADR-0173 §3.3); records memory.retain (№351)" } },
    BuiltClassEntry { name: "memory_release", class: BuiltClass { role: Role::Sink, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "unpins the descendant closure of the key — the surgical idempotent inverse of memory_retain; records memory.release (№351/ADR-0173 §3.3)" } },
    BuiltClassEntry { name: "memory_retained", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Pure, rationale: "lists the pinned keys of the container (sorted introspection; audited) (№351)" } },
    BuiltClassEntry { name: "memory_forget_cascade", class: BuiltClass { role: Role::Sink, default_label: Label::Internal, reversibility: Reversibility::Irreversible, rationale: "THE GRANT-GATED CASCADE FORGET (ADR-0173 §3.4): the ADR-0155 linear action — GRANT_MISSING without a grant, scope memory:forget:<container_id>, grant_use consumption, the post-success irreversible.memory_forget ledger record; delete-class per №316; the delete set is the FULL descendant closure (provenance integrity by construction) and any retained node inside it vetoes the whole forget (MEMORY_RETAIN_PROTECTED)" } },
    BuiltClassEntry { name: "duplex_open", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "opens a duplex channel bound to ONE live session (№348 — SESSION_UNKNOWN otherwise, fail-closed); the priority word is the SESSION ladder (low|normal|high|critical, default normal); the ingress of the barge-in surface; records duplex.open (№352/ADR-0174 §3.3)" } },
    BuiltClassEntry { name: "speak_start", class: BuiltClass { role: Role::Sink, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "starts the SPEAK directed flow — the barge-in engine: same-direction busy refuses (DUPLEX_BUSY), an opposite stream of rank <= incoming is PREEMPTED typed (InterruptedBy; equal wins), a higher-rank opposite refuses (DUPLEX_PREEMPT_DENIED); the text never enters the ledger (digest only); CONSENT-GATED (№428): an active `audio.speak` grant is required — the refusal is typed `AUDIO_CONSENT_REQUIRED` and audited (`duplex.speak_denied`); records duplex.speak_start/preempt/interrupted (№352)" } },
    BuiltClassEntry { name: "listen_start", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "starts the LISTEN directed flow — the symmetric barge-in engine (same-direction DUPLEX_BUSY, opposite preempt typed when rank >= active, DUPLEX_PREEMPT_DENIED otherwise); CONSENT-GATED (№428): an active `audio.listen` grant is required — the refusal is typed `AUDIO_CONSENT_REQUIRED` and audited (`duplex.listen_denied`); records duplex.listen_start/preempt/interrupted (№352)" } },
    ...
    BuiltClassEntry { name: "media_save", class: BuiltClass { role: Role::Sink, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "the ONLY sanctioned materialization of media bytes — file egress through the io sandbox; №325 sink clearance (private-egress) + runtime backstop MEDIA_SEALED_EGRESS, unsealed ONLY by the №387 LikenessToken credential (static presence + registry check; ADR-0162 §2.5 / ADR-0149 D1)" } },
    BuiltClassEntry { name: "media_retain", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    BuiltClassEntry { name: "media_release", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    ...
    BuiltClassEntry { name: "media_bind_origin", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    ...
    BuiltClassEntry { name: "video_understand", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Pure, rationale: "video-understanding backend call (№408, qwen2.5-vl-7b-instruct canon): ingests the comprehension answer for a segment into the flow; no upload, no egress; real mode requires SHA-pinned weights (PARKED №294)" } },
    ...
    BuiltClassEntry { name: "grant_issue", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    BuiltClassEntry { name: "grant_subgrant", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    BuiltClassEntry { name: "grant_revoke", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    BuiltClassEntry { name: "grant_use", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    BuiltClassEntry { name: "db_execute_with_grant", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    ...
    BuiltClassEntry { name: "ledger_count", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    BuiltClassEntry { name: "ledger_head", class: BuiltClass { role: Role::Pure, default_label: Label::Public, reversibility: Reversibility::Pure, rationale: "" } },
    BuiltClassEntry { name: "ledger_export", class: BuiltClass { role: Role::Sink, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "dumps the verifiable JSONL chain to a sandboxed path — FILE EGRESS with an audit event (the signed action trail never leaves the process silently, ADR-0167 §3.5)" } },
    BuiltClassEntry { name: "ledger_export_intoto", class: BuiltClass { role: Role::Sink, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "dumps the in-toto Statement profile (ADR-0157) to a sandboxed path — FILE EGRESS, same class as ledger_export" } },
    BuiltClassEntry { name: "ledger_rotate", class: BuiltClass { role: Role::Lift, default_label: Label::Public, reversibility: Reversibility::Irreversible, rationale: "appends a key-rotation record signed by the still-active key and switches to the fresh key (ADR-0167 §3.3) — the chain transition cannot be undone" } },
    ...
    BuiltClassEntry { name: "ledger_verify", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Pure, rationale: "reads an exported JSONL chain from a sandboxed path and returns the structural verification verdict (№415) — ingress of the signed trail for verification; the runtime ledger is never written and nothing egresses" } },
    BuiltClassEntry { name: "series_make", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "constructs a SeriesHandle over the forecast registry — the numeric payload lives in src/forecast.rs (the ADR-0114 opaque pattern, no values in Value); the source label parses through the №322 lattice (over-tainting allowed, under-tainting impossible); the runtime's own persistent store read (provenance); records forecast.series_make (№440)" } },
    BuiltClassEntry { name: "series_pull", class: BuiltClass { role: Role::Source, default_label: Label::Network, reversibility: Reversibility::Reversible, rationale: "the GRANT-GATED external series pull (№335/№390 contour): an ungrantable argument refuses BEFORE anything else; with an active grant the refusal is the documented no-source-backend gap (Устав §11 Шаг 3) — never a silent substitution; records forecast.pull_denied (№440)" } },
    BuiltClassEntry { name: "forecast_next", class: BuiltClass { role: Role::Source, default_label: Label::Internal, reversibility: Reversibility::Reversible, rationale: "walks the `timeseries` degradation ladder (timesfm-2.5 -> statsforecast -> seasonal_naive) and stores the ForecastHandle with the prov block {window hash, rung/pin, degraded, horizon}; the JOINED source label transfers (LabelJoin — a forecast of a tainted series is tainted); skipped rungs are audited, never silent; records forecast.run (№440/ADR-0165)" } },

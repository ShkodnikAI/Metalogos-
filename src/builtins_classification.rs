...
pub static BUILTIN_CLASSES: &[BuiltClassEntry] = &[
    ...
    BuiltClassEntry { name: "forget", class: BuiltClass { role: Role::Sink, default_label: Label::Internal, reversibility: Reversibility::Irreversible, rationale: "real handler (№445) — the canon §10.3 forgetting front door: the ADR-0155 linear action (scope memory:forget:<container>, grant consumed on success only), dry_run preview → apply, the derived-from cascade → POISONED with closed sinks (MEMORY_POISONED on read/export; the recall lane skips the quarantine), the consent fail-closed gate (MEMORY_FORGET_CONSENT_REQUIRED); records memory.forget {container, targets, hashes, dry_run fact} / memory.forget.denied / irreversible.memory_forget; the legacy 1..2-arg forget(query, days?) surface is unchanged" } },

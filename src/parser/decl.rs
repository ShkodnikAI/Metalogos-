...
pub(super) fn parse_reflex_gen_decl(pair: Pair<Rule>) -> Result<Declaration, ParseError> {
    let children = children_of(&pair);
    let name = find_child_str(&children, Rule::IDENT)
        .ok_or_else(|| pair_error(&pair, "reflex_gen_decl: missing IDENT (model name)"))?;
    let mut input_dim = 0;
    ...
    for child in &children {
        match child.as_rule() {
            ...
            Rule::reflex_vocab_size => {
                let vocab_children = children_of(child);
                for vc in vocab_children {
                    if vc.as_rule() == Rule::INT {
                        vocab_size = vc.as_str().parse::<usize>().unwrap_or(0);
                        has_vocab = true;
                    }
                }
            }
    ...
    if !has_input {
        return Err(pair_error(
            &pair,
            "reflex_gen: 'input' field is required (e.g. input: embedding(64))",
        ));
    }
    if !has_vocab {
        return Err(pair_error(
            &pair,
            "reflex_gen: 'vocab_size' field is required (e.g. vocab_size: 100)",
        ));
    }
    if !has_seed {
        return Err(pair_error(
            &pair,
            "reflex_gen: 'seed' field is required — deterministic weight init",
        ));
    }

    Ok(Declaration::ReflexGen(crate::ast::ReflexGenDecl {
        span: Span::from_pest(pair.as_span()),
        name,
        input_dim,
        vocab_size,

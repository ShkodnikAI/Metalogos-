...
/// Naryad #397 gate parity: a destructive literal in BARE db_execute is a
/// compile-time IRREVERSIBLE_NO_GRANT — the runtime grant algebra meters
/// DELETE/ALTER (grants.rs::extract_destructive_ops), REFERENCE.md and the
/// audit doc comment always claimed the static coverage, and the bare
/// runtime path trusts the static gate (no second runtime gate). The
/// parameterized form is no exception: destructiveness does not depend on
/// the parameters. The granted path (db_execute_with_grant) is the legal
/// route (examples/w2_grant_linear.mlog).
#[test]

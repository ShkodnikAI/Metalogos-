...
use metalogos::embeddings::{
    cosine_similarity, EmbeddingBackend, EmbeddingManager, OpenAIEmbedding,
    OPENAI_EMBEDDING_DIM, TfidfEmbedding,
};
...
fn test_72_memorize_and_recall_shared_words() {
    ...
    // Memorize a fact
    metalogos::feed_line(&mut interp, r#"memorize "the cat sat on the mat" with priority=1.0"#).unwrap();
    ...
    assert!(result.is_some(), "recall flow should return a result for shared words");
    let output = result.unwrap();
    assert!(output.contains("cat sat"), "Should recall 'cat sat', got: {}", output);
}
...
fn test_72_recall_fallback_no_shared_words() {
    ...
    let output = result.unwrap();
    assert!(output.is_empty(), "No shared words should return empty, got: '{}'", output);
}
...
fn test_72_cosine_similarity_same_text() {
    ...
    assert!(sim > 0.9, "Same text should have similarity > 0.9, got {}", sim);
}
...
fn test_72_cosine_similarity_different_text() {
    ...
    assert!(sim < 0.5, "Different domain should have low similarity, got {}", sim);
}
...
fn test_72_embedding_stored_on_memorize() {
    ...
    assert!(!v1.is_empty(), "Embedding for memorize text should not be empty");
    assert!(!v2.is_empty(), "Embedding for recall query should not be empty");

    let sim = cosine_similarity(&v1, &v2);
    assert!(sim < 0.5, "Disjoint vocabularies → low similarity, got {}", sim);
}
...
fn test_72_cosine_similarity_threshold() {
    ...
    assert!(sim > 0.3, "Exact match should exceed default 0.3 threshold, got {}", sim);
}
...
fn test_72_recall_with_knowledge_graph() {
    ...
    metalogos::feed_line(&mut interp, r#"memorize "alice works at google" with priority=1.0"#).unwrap();
    ...
    let output = result.unwrap();
    assert!(output.contains("alice works at google"),
        "Should contain alice entry, got: {}", output);
    assert!(output.contains("[GRAPH]"),
        "Should contain graph traversal, got: {}", output);
}
...
fn test_72_tfidf_partial_overlap() {
    ...
    let v_full = tfidf.embed("the quick brown fox jumps over the lazy dog").unwrap();
    let v_query = tfidf.embed("the quick fox").unwrap();

    let sim = cosine_similarity(&v_query, &v_full);
    assert!(sim > 0.3 && sim < 1.0,
        "Partial overlap should give intermediate similarity, got {}", sim);
}
...
fn test_72_openai_requires_api_key() {
    ...
    assert!(err_msg.contains("METALOGOS_EMBEDDING_API_KEY"), "Error should mention API key, got: {}", err_msg);
}
...
fn test_72_openai_dimension() {
    let emb = OpenAIEmbedding::with_config(
        "sk-test".to_string(),
...
fn test_72_tfidf_unit_norm() {
    let tfidf = TfidfEmbedding::new();
    let v = tfidf.embed("hello world this is a test of the embedding system").unwrap();
    let norm: f32 = v.iter().map(|x| x * x).sum::<f32>().sqrt();
    assert!((norm - 1.0).abs() < 1e-6, "TF-IDF vectors should be unit normalized, got norm={}", norm);
}
...
fn test_72_recall_best_match_among_multiple() {
    ...
    metalogos::feed_line(&mut interp, r#"memorize "the sky is blue" with priority=1.0"#).unwrap();
    metalogos::feed_line(&mut interp, r#"memorize "grass is green" with priority=1.0"#).unwrap();
    metalogos::feed_line(&mut interp, r#"memorize "the ocean is deep blue" with priority=1.0"#).unwrap();
    ...
    assert!(result.is_some());
    let output = result.unwrap();
    assert!(output.contains("blue"), "Should recall a blue-related entry, got: {}", output);
}
